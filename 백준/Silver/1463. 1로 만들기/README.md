...
ex) 9 => %3 , -1 가능 => dp[9/3] 와 dp[9-1]의 value를 temp에 담는다.
...
```
 ...
 temp값 중 비교하여 더 작은 값을 선택 후 , +1 (현재 index까지 오는 횟수를 더해주는 것)을 한 뒤 dp[(현재)index]에 담는다
